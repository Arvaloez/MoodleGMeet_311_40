    ...
    if (isset($googlemeet->addmultiply)) {
        ...
        while ($sdate < $enddate) {
            if ($sdate < $startweek + WEEKSECS) {
                $dayinfo = usergetdate($sdate);
                if (isset($googlemeet->days) && property_exists($googlemeet->days, $wdaydesc[$dayinfo['wday']])) {// Open LMS suggested patch to avoid deprecation message.
                    $event = new stdClass();
                    $event->googlemeetid = $googlemeet->id;
...
function googlemeet_print_recordings($googlemeet, $cm, $context) {
    ...
    // Add GSI library to Sync with Google Drive
    $html = '<head>
